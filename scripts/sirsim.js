...
//the fuction of generaring random number with any average(mu) and variance(sigma)
//reference→https://algorithm.joho.info/programming/javapre-standard-normal-distribution/
...
let sketch = (p) => {
  ...
  let detection;/* Chance that infectious person can be found*/
  let interval;/* Interval until next quarantine is conducted*/
   ...
   * @property {number} defaultValues.interval  -  interval until next quarantine is conducted[100,1000]
   */
  const defaultValues = {
    popsize: 300,
  ...
  *
  *Check if a ball is infectious and, if so, judges it is quarantined or not.
  */
  p.checkafterInfection = () =>{
    let recovery = recoveryTimeInMillis;
    //generating the random number of recoveryTimeInMillis
    recovery = recovery + normRand(recovery,900);
    if(recovery < 0){
      recovery = 0
    }
    ballsInfectionTime.forEach(ball => {
      if (ball.time !== undefined &&
        (Date.now() - ball.time > recovery)) {
        ball.time = undefined;
        ball.quarantime = undefined;
        balls[ball.index].lastStatus = balls[ball.index].status;
        balls[ball.index].status = status.RECOVERED;
      }
      else if (ball.time !== undefined && ball.quarantime !== undefined && Date.now() - ball.quarantime > interval) {
        let fastrandom = fastRandom();
        if(fastRandom < 0){
          fastRandom = 0;
        }
        if(fastrandom < detection){
          ball.time = undefined;
          ball.quarantime = undefined;
          balls[ball.index].lastStatus = balls[ball.index].status;
          balls[ball.index].status = status.QUARANTINED;
        }
        ball.quarantime = Date.now();
      }
    });
  }
  ...
  p.reset = function (args) {
    ...
    detection = args?.detection ?? defaultValues.detection;
    interval = args?.interval ?? defaultValues.interval;
  ...
  class Ball {
    ...
    checkForStatusChange(otherBallIndex) {
      ...
      if (nonVaccinatedInfection || vaccinatedInfection) {
        ...
        ballsInfectionTime.push({ time: Date.now(), index: this.id, quarantime: Date.now() });
      }
    }
